...
def test_start_requests_yields_requests(fake_rss_entry):
    fake_feed = type("FakeFeed", (), {"entries": [fake_rss_entry]})

    with patch("ttd.scraper.rss_article.feedparser.parse", return_value=fake_feed):
        scraper = RSSArticleScraper(start_urls=["https://fake-feed.com"], storage_service=None)
...
def test_parse_article(fake_rss_entry):
    scraper = RSSArticleScraper(start_urls=[], storage_service=None)
    parsed = scraper.parse_article(fake_rss_entry)

    assert parsed["title"] == "AI Breakthrough"
    assert parsed["source_url"] == "https://example.com/ai-news"
